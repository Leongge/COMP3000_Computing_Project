...
class ParamAgent:
    ...
    def make_observation(self, x):
        ...
        if self.initial_cost == 0:
            cost_ratio = 0  # Set a default value
        else:
            cost_ratio = np.clip(self.cost / self.initial_cost, 0, 1)
        self.observation = [
            self.index / 6,
            cost_ratio
        ]
    ...
    def calculate_reward(self, x):
        ...
        plt.cla()
        plt.plot(self.frequency_values,self.s_parameters[self.goals['g0']['series']])
        plt.plot(self.frequency_values,self.s_parameters['S21'])
